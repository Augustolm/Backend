...
routerProduct.get('/cargarProductos'  , async (req, res) => {
  console.log("llege aca")
  try {
    res.render(__dirname + '/views/agregarProductos', {  }); //!delcarar permisos
  } catch (error) {
    res.render(path.join(__dirname, 'views/error'), { errorMessage: `Error al cargar la pantalla: ${error}` });
  }
})
